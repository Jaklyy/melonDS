...
u32 ARMv5::ICacheLookup(const u32 addr)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (((NDS.ARM9Timestamp <= (NDS.WBFinishTime<<NDS.ARM9ClockShift)) && (NDS.ARM9Regions[addr>>14] == NDS.WBLastRegion)) // check write buffer // TODO: REIMPLEMENT!!!!
                || (NDS.ARM9Regions[addr>>14] == DataRegion)) // check the actual store
    ...
    else
    {
        ...
        else
        {
            ...
            else // DCache Streaming logic
            {
                ...
                if ((NDS.ARM9Timestamp <= (NDS.WBFinishTime<<NDS.ARM9ClockShift)) && ((NDS.ARM9Regions[addr>>14]) == NDS.WBLastRegion)) // check write buffer
                    NDS.ARM9Timestamp += 1<<NDS.ARM9ClockShift;
...
void ARMv5::DCacheClearByASetAndWay(const u8 cacheSet, const u8 cacheLine)
{
        ...
        {
            //if (WBDelay > NDS.ARM9Timestamp) NDS.ARM9Timestamp = WBDelay; TODO: REIMPLEMENT!!!!
            ...
            BusWrite32(tag+0x00, ptr[0]);
            BusWrite32(tag+0x04, ptr[1]);
            BusWrite32(tag+0x08, ptr[2]);
            BusWrite32(tag+0x0C, ptr[3]);
            //TimingBlocks[TimingPtr] += 4; //DataCycles += 5; CHECKME: does this function like a write does but with mcr?
        }
        if (DCacheTags[index] & CACHE_FLAG_DIRTY_UPPERHALF) // todo: check how this behaves when both fields need to be written
        {
            ...
            BusWrite32(tag+0x10, ptr[4]);
            BusWrite32(tag+0x14, ptr[5]);
            BusWrite32(tag+0x18, ptr[6]);
            BusWrite32(tag+0x1C, ptr[7]);
...
inline bool ARMv5::WriteBufferHandle()
{
    ...
    if (WBWriting)
    {
        ...
        {
            //TimingBlocks[TimingPtr] = ts & 0xFF;
        }

        if ((WBCurVal >> 61) != 3)
        {
            //WBReleaseTS = WBTimestamp = (ts + ((1<<NDS.ARM9ClockShift)-1)) & ~((1<<NDS.ARM9ClockShift)-1); TODO: REIMPLEMENT!!!!
    ...
    if ((WriteBufferFifo[WBWritePointer] >> 61) != 4) // not an address
    {
        ...
        if (TimingPtr == 0) WBTimestamp = ((NDS.ARM9Timestamp + WBTimestamp + ((1<<NDS.ARM9ClockShift)-1)) & ~((1<<NDS.ARM9ClockShift)-1)) - NDS.ARM9Timestamp;
        else WBTimestamp = (WBTimestamp + (((1<<NDS.ARM9ClockShift)-1)*2) & ~((1<<NDS.ARM9ClockShift)-1)) - ((1<<NDS.ARM9ClockShift)-1);
...
bool ARMv5::DataRead8(u32 addr, u32* val, u8 reg)
{
    if (DCacheFillPtr < 7)
    {
        if (DCacheStreamMainRAM || (TimingPtr > 0))
...
bool ARMv5::DataRead16(u32 addr, u32* val, u8 reg)
{
    if (DCacheFillPtr < 7)
    {
        if (DCacheStreamMainRAM || (TimingPtr > 0))
...
bool ARMv5::DataRead32(u32 addr, u32* val, u8 reg)
{
    if (DCacheFillPtr < 7)
    {
        if (DCacheStreamMainRAM || (TimingPtr > 0))
    ...
    if (addr < ITCMSize)
    {
        ...
        *val = *(u32*)&ITCM[addr & (ITCMPhysicalSize - 1) & ~3];
        return true;
    }
    if ((addr & DTCMMask) == DTCMBase)
    {
        ...
        *val = *(u32*)&DTCM[addr & (DTCMPhysicalSize - 1) & ~3];
        return true;
    }
...
bool ARMv5::DataWrite8(u32 addr, u8 val, u8 reg)
{
    ...
    if (!(PU_Map[addr>>12] & (0x30)))
    {
        ...
        if ((addr >> 24) == 0x02)
        {
            TimingBlocks[TimingPtr++] = 0x8600 | reg; 

            DeferStore[reg][0] = addr;
            DeferStore[reg][1] = val;
        }
        else
        {
            if (TimingPtr > 0)
            {
                TimingBlocks[TimingPtr++] = 0x4400 | reg;
                TimingBlocks[TimingPtr++] = 0x4000 | __builtin_ctz(NDS.ARM9Regions[addr>>14]);
...
bool ARMv5::DataWrite16(u32 addr, u16 val, u8 reg)
{
    ...
    if (!(PU_Map[addr>>12] & 0x30))
    {
        ...
        if ((addr >> 24) == 0x02)
        {
            TimingBlocks[TimingPtr++] = 0x8500 | reg;
...
bool ARMv5::DataWrite32(u32 addr, u32 val, u8 reg)
{
    ...
    if (!(PU_Map[addr>>12] & 0x30))
    {
        ...
        if ((addr >> 24) == 0x02)
        {
            TimingBlocks[TimingPtr++] = 0x8400 | reg;
...
bool ARMv5::DataWrite32S(u32 addr, u32 val, u8 reg)
{
    ...
    if (!(PU_Map[addr>>12] & 0x30)) // non-bufferable
    {
        ...
        if (addr & 0x3FF) // s
        {
            if ((addr >> 24) == 0x02)
            {
                TimingBlocks[TimingPtr++] = 0x8C00 | reg;
            ...
            else
            {
                if (TimingPtr > 0)
                {
                    TimingBlocks[TimingPtr++] = 0x4C00 | reg;
                    TimingBlocks[TimingPtr++] = 0x0000 | __builtin_ctz(NDS.ARM9Regions[addr>>14]);
